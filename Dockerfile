...
RUN apt update \
  ...
  && curl --silent -L https://github.com/Azure/azure-storage-azcopy/releases/download/v10.32.3/azcopy-10.32.3.x86_64.deb -o /tmp/azcopy.dpkg && dpkg -i /tmp/azcopy.dpkg && rm /tmp/azcopy.dpkg \
  && curl --silent -L https://github.com/mikefarah/yq/releases/download/v4.52.4/yq_linux_amd64 -o /usr/local/bin/yq \
  && chmod a+x /usr/local/bin/yq \
  && curl -kLso /tmp/oc.tar.gz https://mirror.openshift.com/pub/openshift-v4/clients/ocp/stable-4.21/openshift-client-linux.tar.gz && tar -xzf /tmp/oc.tar.gz -C /usr/local/bin oc && rm /tmp/oc.tar.gz \
